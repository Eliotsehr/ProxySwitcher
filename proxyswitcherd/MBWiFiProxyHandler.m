...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        handler = [[objc_getClass("MBWiFiProxyHandler") alloc] init];
    });
    return handler;
}

- (void)refreshPreferences { 
    _preferences = [NSDictionary dictionaryWithContentsOfFile:@"/User/Library/Preferences/com.esehr.proxyswitcher.plist"];
}
...
- (void)updateProxy:(BOOL)enabled {
    ...
    if (currentSet) {
        ...
        if (wifiServiceKey) {
            ...
            if (didChange) {
                NSLog(@"Proxy setting did change");
