...
static void networkChanged() {
    ...
    if ([[%c(SBWiFiManager) sharedInstance] wiFiEnabled] && [[%c(SBWiFiManager) sharedInstance] currentNetworkName]) {
        if (type == 1) {
            notify_post("com.esehr.proxyswitcherd.enable"); 
        } else {
            notify_post("com.esehr.proxyswitcherd.disable");
        }
    }
}

static void loadPreferences() {
    CFArrayRef keyList = CFPreferencesCopyKeyList(CFSTR("com.esehr.proxyswitcher"), kCFPreferencesCurrentUser, kCFPreferencesAnyHost);
    NSDictionary *preferences;
    if (keyList) {
        preferences = (__bridge NSDictionary *)CFPreferencesCopyMultiple(keyList, 
                                                                   CFSTR("com.esehr.proxyswitcher"), 
    ...
    notify_post("com.esehr.proxyswitcherd.refreshPreferences");
    setStatusBarVisible(![[[%c(RadiosPreferences) alloc] init] airplaneMode]);
...
- (void)applicationDidFinishLaunching:(id)arg1 {
    %orig;
    statusBarItem = [[%c(LSStatusBarItem) alloc] initWithIdentifier:@"com.esehr.proxyswitcher" alignment:StatusBarAlignmentRight];
...
%ctor {
    ...
    CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), 
                                    ...
                                    CFSTR("com.esehr.proxyswitcher/settingschanged"), 
                                    NULL, 
                                    CFNotificationSuspensionBehaviorCoalesce);
        CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), 
                                    NULL, 
                                    (CFNotificationCallback)toggleProxy, 
                                    CFSTR("com.esehr.proxyswitcheruikit/didTapOnStatusBar"), 
                                    NULL, 
                                    CFNotificationSuspensionBehaviorCoalesce);
}
